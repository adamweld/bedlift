...
int XiaomiCyberGearDriver::init_twai(uint8_t rx_pin, uint8_t tx_pin, bool serial_debug/*=false*/){
    ...
    if (twai_status.state == TWAI_STATE_RUNNING) {
        if (_use_serial_debug) printf("Driver already installed. Skipping init()\n");
        return 0;
    }
    ...
    if (twai_driver_install(&g_config, &t_config, &f_config) == ESP_OK) {
        if (_use_serial_debug) printf("Driver installed\n");
    } else {
        if (_use_serial_debug) printf("Failed to install driver\n");
        return -1;
    }

    //Start TWAI driver
    if (twai_start() == ESP_OK) {
        if (_use_serial_debug) printf("Driver started\n\n");
    } else {
        if (_use_serial_debug) printf("Failed to start driver\n\n");
        return -1;
    }
    ...
    if (twai_reconfigure_alerts(alerts_to_enable, NULL) == ESP_OK) {
        if (_use_serial_debug) printf("CAN Alerts reconfigured\n");
    } else {
        if (_use_serial_debug) printf("Failed to reconfigure alerts\n");
        return -1;
    }
...
void XiaomiCyberGearDriver::_send_can_package(uint8_t can_id, uint8_t cmd_id, uint16_t option, uint8_t len, uint8_t* data){
    ...
    if (twai_transmit(&message, pdMS_TO_TICKS(1000)) == ESP_OK) {
        // if (_use_serial_debug) printf("Message queued for transmission\n\n");
    } else {
        if (_use_serial_debug) printf("Failed to queue message for transmission\n\n");
    }
}
